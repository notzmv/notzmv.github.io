...
## Ditongos Nasais (-ão, -ãe, -õe)
Os ditongos nasais distinguem a **nasalidade etimológica de raiz** da **sufixação derivacional**:
1. **Nasais Etimológicas de Raiz (com Nūn `ن`)**: Monossílabos e palavras primárias com *n* etimológico na raiz latina (*canem*, *panem*, *manum*, *non*) mantêm o **Nūn** (`ن`) do radical antecedendo o ditongo vocálico (`او`, `ای`):
   - *não* $\rightarrow$ **`ناو`** (*non* $\rightarrow$ Nūn `ن` + `-ão` `او`)
   - *cão* $\rightarrow$ **`كناو`** (*canem* $\rightarrow$ Kāf `ك` + Nūn `ن` + `او`; plural *cães* $\rightarrow$ **`كنایها`**)
   - *pão* $\rightarrow$ **`پناو`** (*panem* $\rightarrow$ Pā `پ` + Nūn `ن` + `او`)
   - *mão* $\rightarrow$ **`مناو`** (*manum* $\rightarrow$ Mīm `م` + Nūn `ن` + `او`)
2. **Sufixos Derivacionais em *-ção*, *-dão*, *-são***: Nos sufixos nominais derivacionais (ex.: *coração*, *gratidão*, *atenção*, *ação*, *convenção*), o ditongo nasal em *-ão* grafar-se-á com **`ـاو`** (*Alif-Waw*) anexado diretamente à consoante da raiz/sufixo, sem adicionar um Nūn (`ن`) epentético redundante:
...
| **cão** | *cães* | `كناو` | `كنایها` | Ditongo nasal *-ães* $\rightarrow$ `ایها` |
