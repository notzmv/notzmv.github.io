...
- **G brando** (antes de *e*, *i*, representando a fricativa palatal sonora /ʒ/) mapeia para **Jīm** (`ج`):
  ...
  - *gerúndio* $\rightarrow$ `جرندیو` / `ژرندیو` (preservando o hiato/ditongo proparoxítono `-dio` `دیو`)
...
- **nh** [ɲ] $\rightarrow$ **نی**: *minha* $\rightarrow$ `مینیہ`, *tenho* $\rightarrow$ `تنهو` (paroxítona, sem Ye `ی` no radical), *linhas* $\rightarrow$ `لینیها` / `لینی-ها` (`liniha` ou `lini-ha` com sufixo `-hā`), *desenho* $\rightarrow$ `دزنیو`
